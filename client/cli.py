...
def put_file(path, user="demo"):
    filename = f"/user/{user}/{os.path.basename(path)}" # así guardamos el archivo en el nameNode con toda su ruta, para usar ls
...
def get_file(filename, outpath, user="demo"):
...
if __name__ == "__main__":
    ...
    p_put.add_argument("--user", default="demo", help="Usuario")
    ...
    p_get.add_argument("--user", default="demo", help="Usuario")
    ...
    if args.cmd == "put":
        ...
        get_file(args.filename, args.outpath, args.user)
    elif args.cmd == "ls":
        r = requests.get(f"{NAMENODE}/namenode/ls", params={"path": args.path})
        if r.status_code == 200:
            data = r.json()
